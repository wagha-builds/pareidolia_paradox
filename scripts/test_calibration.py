...
def main():
    ...
    print(
        f"Dataset: {len(images)} images | Prior pi1 (Rise): {labels.mean():.4f}",
        flush=True,
    )
    print(
        f"Class 0 (Depth): {(labels == 0).sum()} images | Class 1 (Rise): {(labels == 1).sum()} images",
        flush=True,
    )
    ...
    if az_ba > 0.55:
        print(
            f"  [NOTE] BA={az_ba:.4f} > 0.55 — azimuth distribution alone predicts label.",
            flush=True,
        )
        print(
            "         This is a known label-leakage concern for FiLM conditioning.",
            flush=True,
        )
    ...
    print(
        f"  Depth (Class 0): mode_delta={mode_c0:6.2f}°  R={R_c0:.4f}  n={len(phi_c0)}",
        flush=True,
    )
    print(
        f"  Rise  (Class 1): mode_delta={mode_c1:6.2f}°  R={R_c1:.4f}  n={len(phi_c1)}",
        flush=True,
    )
    print(
        f"  Angular separation between class modes: {separation:.2f}° (ideal ~180°)",
        flush=True,
    )
    ...
    print(
        "  If images are North-up orbital tiles, this is the expected convention.",
        flush=True,
    )
    ...
    print(
        f"  Physical (delta=90, s=-1): mode={mode_phys:.2f}°  R={R_phys:.4f}",
        flush=True,
    )
    print(
        "  (If this R is high and mode~90°, the physical convention is correct)",
        flush=True,
    )
    ...
    print("=" * 60, flush=True)
    print(
        f"  Combined calibration best R:  {best['R']:.4f}  (threshold: 0.40)",
        flush=True,
    )
    ...
    print(
        f"  Class mode separation:        {separation:.1f}°  (ideal: 180°)", flush=True
    )
