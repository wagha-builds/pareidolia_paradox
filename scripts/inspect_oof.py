...
for r in runs:
    ...
    print(f"  {r}")
    print(
        f"    n={len(valid)} | min={valid.min():.4f} max={valid.max():.4f} mean={valid.mean():.4f} std={valid.std():.6f}"
    )
...
for m in manifests:
    ...
    print(f'  {d["run_id"]}')
    print(
        f'    OOF_BA={d["metrics"]["oof_ba"]:.4f}  threshold={d["metrics"]["threshold"]:.4f}  AUC={d["metrics"]["oof_auc"]}'
    )
    for fm in d.get("fold_metrics", []):
        print(
            f'    fold={fm["fold"]}  best_ba@0.5={fm["best_ba_at_0_5"]:.4f}  best_epoch={fm["best_epoch"]}'
        )
