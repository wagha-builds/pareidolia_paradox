...
def main():
    ...
    delta, s, R = (
        float(best_calib["delta"]),
        int(best_calib["s"]),
        float(best_calib["R"]),
    )
    print(
        f"Best Calibration output (Crop {best_crop}): delta={delta:.2f}, s={s}, R={R:.4f} (other R={best_calib['R_other_s']:.4f})"
    )
    ...
    for fold, (_, val_idx) in enumerate(
        sgkf.split(train_df, train_df["label"], train_df["group_id"])
    ):
