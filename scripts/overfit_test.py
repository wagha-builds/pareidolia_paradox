...
print(f"Expected: loss should drop well below 0.4 by epoch 30\n")
...
for epoch in range(1, 61):
    ...
    if epoch % 5 == 0 or epoch == 1:
        ...
        if mean_loss < 0.3 and ba > 0.75:
            print(f"  [PASS] Model can overfit. Training pipeline is healthy.")
            break

print("\nFinal diagnosis:")
if mean_loss > 0.55:
    print("  [FAIL] BROKEN: Loss is stuck near log(2)=0.693. Model cannot overfit 64 images.")
    print("     Likely causes: normalization wrong, data pipeline bug, or model init issue.")
elif mean_loss > 0.35:
    print("  ⚠️  SLUGGISH: Loss improving but slow. LR or weight decay may need tuning.")
else:
    print("  ✅ HEALTHY: Pipeline is fine. Training collapse is a hyperparameter/config issue.")
    print("     Fix: add LR warmup, reduce LR for full run, or check early stopping patience.")
